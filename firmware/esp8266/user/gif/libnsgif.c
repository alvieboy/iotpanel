...
*/
#define GIF_PROCESS_COLOURS 0xaa
...
*/
void ICACHE_FLASH_ATTR gif_create(gif_animation *gif, gif_bitmap_callback_vt *bitmap_callbacks) {
	...
	if (gif->global_colour_table[0] == GIF_PROCESS_COLOURS) {
		...
		*/
		if (gif->global_colours) {
			...
			for (index = 0; index < gif->colour_table_size; index++) {
				...
				unsigned char *entry = (unsigned char *) &gif->
						global_colour_table[index];
                                unsigned char red = gif_data[0]>>5;
                                unsigned char green = gif_data[1]>>5;
                                unsigned char blue = gif_data[2]>>6;
                                green<<=3;
                                blue <<=6;
			...
			*/
			pixel_t *entry = gif->global_colour_table;
...
/**	Updates the sprite memory size
	...
	@return GIF_INSUFFICIENT_MEMORY for a memory error
		GIF_OK for success
*/
static gif_result ICACHE_FLASH_ATTR  gif_initialise_sprite(gif_animation *gif, unsigned int width, unsigned int height) {
...
/**	Attempts to initialise the next frame
	...
	@return GIF_INSUFFICIENT_DATA for insufficient data to do anything
		...
		GIF_WORKING for successful decoding if more frames are expected
*/
static gif_result ICACHE_FLASH_ATTR  gif_initialise_frame(gif_animation *gif) {
	int frame;
	gif_frame *temp_buf;
...
*/
static gif_result ICACHE_FLASH_ATTR  gif_initialise_frame_extensions(gif_animation *gif, const int frame) {
	unsigned char *gif_data, *gif_end;
	int gif_bytes;
...
*/
gif_result  ICACHE_FLASH_ATTR gif_decode_frame(gif_animation *gif, unsigned int frame) {
	unsigned int index = 0;
	unsigned char *gif_data, *gif_end;
	int gif_bytes;
	unsigned int width, height, offset_x, offset_y;
	unsigned int flags, colour_table_size, interlace;
	pixel_t *colour_table;
	pixel_t *frame_data = 0;	// Set to 0 for no warnings
	pixel_t *frame_scanline;
	...
	if (!clear_image) {
		...
		*/
		if ((frame == 0) || (gif->decoded_frame == GIF_INVALID_FRAME)) {
			memset((char*)frame_data, GIF_TRANSPARENT_COLOUR, gif->width * gif->height * sizeof(pixel_t));
		...
		*/
		if (gif->frames[frame].disposal_method == GIF_FRAME_CLEAR) {
			for (y = 0; y < height; y++) {
				...
				if (gif->frames[frame].transparency)
					memset(frame_scanline, GIF_TRANSPARENT_COLOUR, width * sizeof(pixel_t));
				else
					memset(frame_scanline, colour_table[gif->background_index], width * sizeof(pixel_t));
			}
		}
	}
...
/**	Skips the frame's extensions (which have been previously initialised)
	...
	@return GIF_INSUFFICIENT_FRAME_DATA for insufficient data to complete the frame
		GIF_OK for successful decoding
*/
static gif_result ICACHE_FLASH_ATTR  gif_skip_frame_extensions(gif_animation *gif) {
	unsigned char *gif_data, *gif_end;
	int gif_bytes;
	unsigned int block_size;
...
static unsigned int  ICACHE_FLASH_ATTR gif_interlaced_line(int height, int y) {
...
*/
void  ICACHE_FLASH_ATTR gif_finalise(gif_animation *gif) {
 ...
 */
void  ICACHE_FLASH_ATTR gif_init_LZW(gif_animation *gif) {
	int i;
...
static bool  ICACHE_FLASH_ATTR gif_next_LZW(gif_animation *gif) {
	int code, incode;
	int block_size;
	int new_code;
...
static int  ICACHE_FLASH_ATTR gif_next_code(gif_animation *gif, int code_size) {
